...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.config = config
        global WALL, SUPPORT, TURRET, SCOUT, DEMOLISHER, INTERCEPTOR, MP, SP, do_not_build, corner
    ...
    def strategy(self, game_state):
        ...
        if self.scan1(game_state):
            do_not_build.append([6, 10])
            game_state.attempt_remove(do_not_build)
        elif [6, 10] in do_not_build:
            do_not_build.remove([6, 10])

        if len(self.miss_defend_line(game_state)) == 0:
            number_enemy_support = self.scan_enemy_for_units(game_state, "EF")

            divisor = max(8 - number_enemy_support, 4)
            game_state.attempt_spawn(INTERCEPTOR, defend_locations[0], int(
                game_state.get_resource(MP, 1) / divisor))

        if self.damage_spawn_location(game_state, attack_locations[0]) == 0 and game_state.get_resource(SP, 1) <= 5:
            self.support(game_state)
            if len(self.miss_defend_line(game_state)) == 0:
                game_state.attempt_spawn(SCOUT, attack_locations[0], 1000)
            else:
            ...
            if closest_enemy_turret_row == -1 or closest_enemy_turret_row == 3:
                if self.optimal_spawn(game_state):
                    game_state.attempt_spawn(
                        DEMOLISHER, self.optimal_spawn(game_state), 1000)
                else:
                    game_state.attempt_spawn(
                        DEMOLISHER, attack_locations[0], 1000)
                ...
                game_state.attempt_spawn(DEMOLISHER, attack_locations[0], 1000)
        elif game_state.turn_number % 5 == 0 and game_state.turn_number <= 20:
            ...
            if closest_enemy_turret_row == -1 or closest_enemy_turret_row == 3:
                ...
                self.support(game_state)
                game_state.attempt_spawn(DEMOLISHER, attack_locations[0], 1000)
        elif game_state.turn_number % 4 == 0 and game_state.turn_number > 20:
            if not self.horizontal(game_state):
                self.vertical(game_state)
            if len(self.miss_defend_line(game_state)) != 0:
                self.save_me(game_state, self.miss_defend_line(game_state))
    ...
    def build(self, game_state, buildings):
        for building in buildings:
            if building[1] not in do_not_build:
    ...
    def repair(self, game_state, buildings):
        ...
        for building in buildings:
            if game_state.game_map[building[1]]:
                if 2 * game_state.game_map[building[1]][0].health <= game_state.game_map[building[1]][0].max_health and game_state.game_map[building[1]][0].health != 60:
                    gamelib.debug_write('Health {}'.format(
                        game_state.game_map[building[1]][0].health))
                    gamelib.debug_write('Max Health {}'.format(
                        game_state.game_map[building[1]][0].max_health))
    ...
    def support_only_row_1(self, game_state):  # Spawn location is at [2, 11]
        support = [
            [SUPPORT, [1, 12], True],
            [SUPPORT, [2, 12], True],
            [SUPPORT, [3, 10], True],
    ...
    def save_me(self, game_state, list):
        ...
        if game_state.get_resource(SP, 0) > 7:
            return

        for i in preference:
            if i in list:
                game_state.attempt_spawn(INTERCEPTOR, i, int(
                    game_state.get_resource(MP, 1) / 6))
                break
